      ...
      const sampleCategories: BlogCategory[] = [
        {
          ...
          slug: "tech-ai",
          description: "Статьи о ��овых технологиях и искусственном интеллекте",
          color: "bg-blue-600",
          postCount: 0,
        },
        {
    ...
    if (shouldInitialize) {
      ...
      const samplePosts: BlogPost[] = [
        ...
        },
        {
          ...
          excerpt:
            "Как использовать ИИ для более глубокого анализа ретроспективных встреч в Agile-��омандах.",
          heroImage:
  ...
  private getSampleContent(type: string): string {
    const contentTemplates = {
      ...
      "ai-future": `
        ...
        <h2 id="intro">Введение</h2>
        <p>По данным исследований, 67% руководителей считают, что ИИ кардинально изменит деловые комм��никации в ближайшие 5 лет. Мы находимся на пороге революции в том, как проводятся встречи, принимаются решения и документируются результаты.</p>
      ...
      `,
      "vr-meetings": `
        <p class="lead">Виртуальная реальность открывает нов��е горизонты для деловых встреч. Современные VR-платформы позволяют создавать иммерсивные пространства для командной работы, где участники могут взаимодействовать с 3D-объектами и совместно решать задачи.</p>
